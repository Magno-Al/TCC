...
A leitura do MAX30102 foi realizada por meio da interface \textit{/dev/i2c} exposta nativamente pelo \textit{kernel} Linux. A partir do sinal PPG bruto adquirido, dois parâmetros foram extraídos de forma direta, a frequência cardíaca (FC), obtida pela detecção de picos no componente pulsátil (AC) do sinal, e a saturação de oxigênio (SpO\textsubscript{2}), calculada pela razão de absorção entre os comprimentos de onda vermelho e infravermelho emitidos pelo sensor \cite{allen2007photoplethysmography}. A frequência respiratória (FR) foi estimada a partir das modulações respiratórias presentes no sinal PPG — variação de linha de base, modulação de amplitude e modulação de frequência dos intervalos inter-batimento —, seguindo as abordagens descritas por \citeonline{charlton2018breathing}. A leitura da temperatura corporal foi realizada via \textit{driver} \textit{w1-therm}, nativo do \textit{kernel} Linux, que expõe o valor medido pelo DS18B20 diretamente por meio do sistema de arquivos em \textit{/sys/bus/w1/devices}.
